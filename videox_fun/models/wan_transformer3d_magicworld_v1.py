...
class WanTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            WanAttentionBlock(cross_attn_type, dim, ffn_dim, num_heads,
                              window_size, qk_norm, cross_attn_norm, eps, use_history)
        ...
        if use_history:
            print("In this stage, we use history frame !!!!!!!!!")
            self.history_patch_embedding = zero_module(nn.Conv3d(16, dim, kernel_size=patch_size, stride=patch_size))
        else:
            self.history_patch_embedding = None
        ...
        if use_history:
            with torch.no_grad():
                self.history_patch_embedding.weight.copy_(self.patch_embedding.weight.data[:, :16, :, :, :])
                self.history_patch_embedding.bias.copy_(self.patch_embedding.bias)
    ...
    def forward(
        ...
        if y_history is not None:
            history_frames = y_history.size(2)
            history_token = self.history_patch_embedding(y_history).flatten(2).transpose(1, 2) 
            grid_sizes = torch.stack([torch.tensor([u[0] + history_frames, u[1], u[2]]) for u in grid_sizes]).to(grid_sizes.device)
            seq_len += history_token.size(1)
            x = [torch.concat([u, _history_token.unsqueeze(0)], dim=1) for _history_token, u in zip(history_token, x)]
            if t.dim() != 1 and t.size(1) < seq_len:
                pad_size = seq_len - t.size(1)
                last_elements = t[:, -1].unsqueeze(1)
                padding = last_elements.repeat(1, pad_size)
        ...
        if y_history is not None:
            history_token_length = history_token.size(1)
            x = x[:, :-history_token_length]
            grid_sizes = torch.stack([torch.tensor([u[0] - history_frames, u[1], u[2]]) for u in grid_sizes]).to(grid_sizes.device)
