...
class Camera(object):
    ...
    def __init__(self, entry):
        if len(entry) == 19:
            ...
            # === 高精度格式：K (3x3) + T (4x4) ===
            K = np.array(entry[0:9]).reshape(3, 3)
            fx = K[0, 0]
            fy = K[1, 1]
            cx = K[0, 2]
            cy = K[1, 2]
            ...
            w2c_mat_4x4 = np.array(entry[9:]).reshape(4, 4)
            self.w2c_mat = w2c_mat_4x4
            self.c2w_mat = np.linalg.inv(w2c_mat_4x4)

        else:
            raise ValueError(f"Unsupported entry length: {len(entry)}")
...
def process_pose_file_interactive(cam_params, width=672, height=384, original_pose_width=1280, original_pose_height=720, device='cpu'):
    """Modified from https://github.com/hehao13/CameraCtrl/blob/main/inference.py
    """
    cam_params = [Camera(cam_param) for cam_param in cam_params]
...
class ImageVideoCameraDepthControlDataset(Dataset):
    def __init__(
        self,
        ann_path, data_root=None,
        video_sample_size=512, video_sample_stride=4, video_sample_n_frames=16,
        ...
        self.video_depth_transforms = transforms.Compose(
            [
    ...
    def get_batch(self, idx):
        ...
        if data_info.get('type', 'image')=='video':
            ...
            ###### process depth video  ######
            depth_video_id = data_info['depth_path']
            if self.data_root is None:
            ...
            with VideoReader_contextmanager(depth_video_id, num_threads=2) as depth_video_reader:
                ...
                if not self.enable_bucket:
                    depth_pixel_values = self.video_depth_transforms(depth_pixel_values)
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            try:
                ...
                pixel_values, depth_pixel_values, control_pixel_values, control_camera_values, name, data_type, file_path = self.get_batch(idx)
...
class ImageVideoCameraControlDataset(Dataset):
    def __init__(
        self,
        ann_path, data_root=None,
        video_sample_size=512, video_sample_stride=4, video_sample_n_frames=16,
...
class ImageVideoCameraInpControlDataset(Dataset):
    def __init__(
        self,
        ann_path, data_root=None,
        video_sample_size=512, video_sample_stride=4, video_sample_n_frames=16,
