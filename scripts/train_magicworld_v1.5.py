...
from videox_fun.models.wan_transformer3d_world_model import WanTransformer3DModel
...
from videox_fun.utils.utils import (get_image_to_video_latent,
                                    ...
                                    save_videos_grid)
from videox_fun.utils.flowproxyloss import OnlineRAFTFlowProxy
...
def resize_mask(mask, latent, process_first_frame_only=False):
...
def log_validation(vae, text_encoder, tokenizer, clip_image_encoder, transformer3d, args, config, accelerator, weight_dtype, global_step):
    try:
        ...
        transformer3d_val = WanTransformer3DModel.from_pretrained(
            os.path.join(args.pretrained_transformer_path, config['transformer_additional_kwargs'].get('transformer_subpath', 'transformer')),
...
def parse_args():
    ...
    parser.add_argument(
        "--motion_flow_plus_loss", action="store_true", help="Whether enable motion sub loss."
    )
    parser.add_argument(
        "--motion_flow_plus_ratio", type=float, default=0.0001, help="The ratio of motion sub loss."
    )
    parser.add_argument(
...
def main():
    ...
    ## customize transformer3d patch_embedding in_dim
    new_patch_embedding = nn.Conv3d(
    in_channels=16+16+16,
    out_channels=transformer3d.patch_embedding.out_channels,
    kernel_size=transformer3d.patch_embedding.kernel_size,
    stride=transformer3d.patch_embedding.stride
    )
    torch.nn.init.kaiming_normal_(new_patch_embedding.weight)
    new_patch_embedding.weight.data = new_patch_embedding.weight.data * 0.
    old_weight = transformer3d.patch_embedding.weight.data
    new_patch_embedding.weight.data[:, :old_weight.size(1), :, :, :] = old_weight
    new_patch_embedding.bias.data = transformer3d.patch_embedding.bias.data
    transformer3d.patch_embedding = new_patch_embedding
    if isinstance(transformer3d.config, dict):
        transformer3d.config['in_dim'] = 16+16+16
    else:
        transformer3d.config.in_dim = 16+16+16
    ## customize end
    ...
    # Get the dataset
    train_dataset = ImageVideoCameraControlDataset(
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            new_examples["render_mask_values"] = []
            new_examples["render_pixel_values"] = []
            ...
            for example in examples:
                ...
                render_mask_values = torch.from_numpy(example["render_mask_values"]).permute(0, 3, 1, 2).contiguous() #f, c, h, w
                render_mask_values = render_mask_values / 255.
                render_mask_values = render_mask_values[:,0:1]
                render_mask_values[render_mask_values < 0.5] = 0
                render_mask_values[render_mask_values >= 0.5] = 1

                render_pixel_values = torch.from_numpy(example["render_pixel_values"]).permute(0, 3, 1, 2).contiguous() #f, c, h, w
                ...
                new_examples["render_pixel_values"].append(transform(render_pixel_values))
                new_examples["render_mask_values"].append(torch.nn.functional.interpolate(render_mask_values, size=resize_size))
            ...
            new_examples["render_pixel_values"] = torch.stack([example[:batch_video_length] for example in new_examples["render_pixel_values"]])
            new_examples["render_mask_values"] = torch.stack([example[:batch_video_length] for example in new_examples["render_mask_values"]])
    ...
    ######################
    ######################
    proxy_module = OnlineRAFTFlowProxy(device=accelerator.device, short_side=320, stride=4)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if epoch == first_epoch and step == 0:
                ...
                render_pixel_values = batch["render_pixel_values"].cpu()
                render_mask_values = batch["render_mask_values"].cpu()
                control_pixel_values = batch["control_pixel_values"].cpu()
                
                
                pixel_values = rearrange(pixel_values, "b f c h w -> b c f h w")
                render_pixel_values = rearrange(render_pixel_values, "b f c h w -> b c f h w")
                render_mask_values = rearrange(render_mask_values, "b f c h w -> b c f h w")
                control_pixel_values = rearrange(control_pixel_values, "b f c h w -> b c f h w")
                
                
                os.makedirs(os.path.join(args.output_dir, "sanity_check"), exist_ok=True)
                for idx, (pixel_value, render_pixel_value, render_mask_value, control_pixel_value, text) in enumerate(zip(pixel_values, render_pixel_values, render_mask_values, control_pixel_values, texts)):
                    pixel_value = pixel_value[None, ...]
                    render_pixel_value = render_pixel_value[None, ...]
                    render_mask_value = render_mask_value[None, ...]
                    ...
                    save_videos_grid(render_pixel_value, f"{args.output_dir}/sanity_check/{gif_name[:10]}_render.gif", rescale=True)
                    save_videos_grid(render_mask_value, f"{args.output_dir}/sanity_check/{gif_name[:10]}_render_mask.gif", rescale=True)
            ...
            with accelerator.accumulate(transformer3d):
                ...
                render_pixel_value = batch["render_pixel_values"].to(weight_dtype)
                render_mask_value = batch["render_mask_values"].to(weight_dtype)
                ...
                if args.random_frame_crop:
                    ...
                    render_pixel_value = render_pixel_value[:, :temp_n_frames, :, :]
                    render_mask_value = render_mask_value[:, :temp_n_frames, :, :]
                ...
                if args.keep_all_node_same_token_length:
                    ...
                    render_pixel_value = render_pixel_value[:, :actual_video_length, :, :]
                    render_mask_value = render_mask_value[:, :actual_video_length, :, :]
                ...
                with torch.no_grad():
                    ...
                    def _pick_equal_segment_3(
                        latents: torch.Tensor,
                        seg_len: int = 3,
                        generator: Optional[torch.Generator] = None,
                    ) -> Tuple[torch.Tensor, int, Tuple[int, int]]:
                        assert latents.ndim == 5, "latents 应为 [B, C, T, H, W]"
                        B, C, T, H, W = latents.shape
                        assert T >= seg_len, f"T({T}) 必须 >= seg_len({seg_len})"
                        num_segments = T // seg_len               # 动态段数
                        assert num_segments > 0, "有效段数为 0"
                        # 随机选择一个段
                        if generator is None:
                            seg_id = torch.randint(num_segments, (1,), device=latents.device).item()
                        else:
                            seg_id = torch.randint(num_segments, (1,), generator=generator, device=latents.device).item()
                        s = seg_id * seg_len
                        e = s + seg_len
                        chunk = latents[:, :, s:e, :, :]          # 视图切片
                        return chunk, seg_id, (s, e)
                    ...
                    if args.train_mode != "control_camera_ref":
                        ...
                        #### process render video and render mask ####
                        render_latents = _batch_encode_vae(render_pixel_value)
                        # Make render latents to zero
                        for bs_index in range(render_latents.size()[0]):
                            if rng is None:
                                zero_init_control_latents_conv_in = np.random.choice([0, 1], p = [0.90, 0.10])
                            else:
                                zero_init_control_latents_conv_in = rng.choice([0, 1], p = [0.90, 0.10])
                            ...
                            if zero_init_control_latents_conv_in:
                                render_latents[bs_index] = render_latents[bs_index] * 0

                        mask_bs, _, _, mask_h, mask_w = render_mask_value.size()
                        render_mask_condition = rearrange(render_mask_value, "b f c h w -> b c f h w")
                        render_mask_condition = torch.concat(
                        [
                            torch.repeat_interleave(render_mask_condition[:, :, 0:1], repeats=4, dim=2), 
                            render_mask_condition[:, :, 1:]
                        ], dim=2
                        )
                        render_mask_condition = render_mask_condition.view(mask_bs, render_mask_condition.shape[2] // 4, 4, mask_h, mask_w)
                        render_mask_condition = render_mask_condition.transpose(1, 2)
                        render_mask_condition = resize_mask(render_mask_condition, render_latents, False)
                        is_binary = torch.all(torch.logical_or(render_mask_condition == 0, render_mask_condition == 1))
                        assert is_binary, "mask_condition contains values must be 0 or 1."
                    ...
                    if args.train_mode != "control":
                        ...
                        if render_latents is not None:
                            control_latents = torch.cat([control_latents, render_latents], dim = 1)
                ...
                if args.train_mode == "control_camera_ref":
                    history_cond, seg_id, (s, e) = _pick_equal_segment_3(latents)

                    # os.makedirs("debug_history_cond", exist_ok=True)
                    # print("begin save!!!!!!!")

                    # with torch.no_grad():
                    #     B, C, T, H, W = history_cond.shape
                    ...
                    #         with torch.no_grad():
                    #             frames = vae.decode(latents).sample           # [N, 3, H, W] or [N, 3, T, H, W]
                    #             frames = (frames / 2 + 0.5).clamp(0, 1)

                    #         # 转回 CPU + float32 + numpy，方便后面用 PIL 存图
                    #         frames = frames.cpu().float().numpy()
                    #         return frames
                        
                    #     def save_frames_matplotlib(frames, out_dir="debug_history_cond"):
                    ...
                    #                 plt.axis("off")
                    #                 plt.savefig(os.path.join(out_dir, f"sample_{b:03d}_frame_{t:04d}.png"), bbox_inches="tight", pad_inches=0)
                    #                 plt.close()

                    #     # [B, C, T, H, W] -> [B, T, C, H, W] -> [B*T, C, H, W]

                    #     frames = decode_latents(history_cond) #[B,C,T,H,W]
                    #     print(frames.shape)
                    #     save_frames_matplotlib(frames)
                ...
                with torch.cuda.amp.autocast(dtype=weight_dtype), torch.cuda.device(device=accelerator.device):
                    noise_pred = transformer3d(
                        ...
                        y_history=history_cond if args.train_mode == "control_camera_ref" else None,
                ...
                if args.motion_flow_plus_loss:
                    # x0_pred_latent for rectified-flow
                    # print("Using proxy loss")
                    x0_pred_latent = noisy_latents - sigmas * noise_pred  # [B,C,21,H,W] 
                    sigma_scalar = sigmas.flatten()[0].item()
                    # print("sigma_scalar, sigmas ", sigma_scalar, sigmas)
                    # print("!!!!!!!!!!!!!!!!!!!!!!",sigmas,sigma_scalar)
                    if sigma_scalar < 0.2:
                        proxy_loss = proxy_module(pixel_values, x0_pred_latent.float(), occl_thresh=1.0)
                        # loss = loss * (1 - args.motion_flow_plus_ratio) + proxy_loss * args.motion_flow_plus_ratio
                        # print("loss", loss)
                        # print("loss_proxy", proxy_loss)
                        loss = loss + args.motion_flow_plus_ratio * proxy_loss
